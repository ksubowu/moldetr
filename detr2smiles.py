...
# data_dir='/home/tangbw/works/datas/chembl5k/images'
...
def merge_dataframes_by_file_paths(dataframes, file_paths):
    ...
    for name, df in dataframes.items():
        if 'file_path' in df.columns:
            # 过滤 file_path 列值在 file_paths 列表中的行
...
def read_csv_to_dataframes(*csv_files):
    ...
    for file_path in csv_files:
        try:
            ...
            # 使用文件名（去掉 .csv 扩展名）作为 DataFrame 的键
            file_name = file_path.split('/')[-1].replace('.csv', '')
            df['file_path']=f'{file_name}_'+df['file_path'].str.split('/').str[-1]# add prefix
...
# csv_files = [
#     "/home/tangbw/works/datas/acs.csv",
#     "/home/tangbw/works/datas/JPO.csv",
#     "/home/tangbw/works/datas/UOB.csv",
#     "/home/tangbw/works/datas/USPTO.csv",
#     "/home/tangbw/works/datas/CLEF.csv"
# ]
# # 调用函数读取 CSV 文件并创建 DataFrame
# dataframes = read_csv_to_dataframes(*csv_files)
# # 打印每个 DataFrame 的基本信息
# for name, df in dataframes.items():
#     print(f"\nDataFrame: {name}")
#     # print("前2行数据：")
#     print(df.head(2))
...
# 根据 file_paths 合并 DataFrame
# merged_df_R4 = merge_dataframes_by_file_paths(dataframes, file_paths)


# merged_df_R4.to_csv('R4_output.csv', index=False)
...
if other2ppsocr:
    ...
    ocr = TextRecognition(model_name="PP-OCRv4_server_rec")#,lan='en')
    # ocr = PaddleOCR(
    ...
    # )
    # lang='latin',
    # lang="en",  use_debug=False, 
    # ocr = PaddleOCR(
    #     use_angle_cls='use_textline_orientation',
    #     # rec_algorithm='SVTR_LCNet',
    #     rec_model_dir='/home/tangbw/.paddleocr/whl/rec/en/en_PP-OCRv4_rec_infer',
    #             lang="en") 
...
for ff in pngs:
    ...
    # if [w_ori, h_ori]!=[256,256]:#only for staker
    ...
    if other2ppsocr:
        ...
        for i, atc in enumerate(atom_classes):
            if 'other' == atc:  # 30 idx_lab version OH-->Cl with high 
                ...
                result_ocr_scaled = ocr.predict(image_npocr_scaled)#, det=False)
    ...
    if getacc:
        ...
        if sameWithOutStero or sameWithOutStero_exp or logest_flag or logest_flagHfix:
            ...
            if ac_b:
                ...
                with open(b_datadir,"w") as wf2:
                        json.dump(b_dict, wf2, indent=2)
            if acn and (atom_num!=aclas_num):#atom charge not equal
            ...
            if bn and (bond_num!=bclas_num):#atom charge not equal
